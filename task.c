...
int size;
 ...
 void print_Gaussian(const char* s) {
	int i, j;
	puts(s);
	 for (i = 0; i < size; i++) {
	 for (j = 0; j < size; j++) {
...
int main(int argc, char* argv[]) {
	...
	printf("\nGaussian elimination used %ld sec\n", (time_sec_finish - time_sec_start));
	double sumax = 0;
	...
	print_Gaussian("After Gaussian Elimination **** ");
	puts("the result!\n *********************************\n");
...
//Results
	for (i = 0; i < size; i++) {
		for (j = 0; j < size; j++) {
			printf("%.2f ", test_Matrix[i][j]);
		}
		printf("   %10.2f", vector_x[i]);
		printf("   %.2f", test_Vector[i]);
		printf(" diff %f\n", temp);
	}
	l2 = sqrt(l2);
